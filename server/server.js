                ...
                model.connect(function (db,client) {
                    ...
                    db.collection("article").updateOne({textno:data.title},{$set:{textno:data.title,            //更新article中的文章
                        ...
                        if(err){
                            ...
                            conn.sendText(JSON.stringify({type:"0",text:"更新成功！"}));
                            flags1=1;
                            console.log(data.contentofrefection);
                            db.collection("reflection").updateOne({textno:data.title+""},{$set:{textno:data.title,content:data.contentofrefection}});
                        }
                    })
                    ...
                    db.collection("dataarrays").find({textno:data.title}).toArray(function (err,ret) {
                        if(err){
                            ...
                            console.log("finalnums的结果为",finalnums);
                            db.collection("dataarrays").updateOne({textno:data.title},{$set:{contributions:finalnums}},function (err,ret) {
                                if(err){
                                    ...
                                    db.collection("timelinecao").insertOne({textno:data.title,timestamp:new Date(Date.now()+8*60*60*1000),authors:authors,contributions:finalnums})
                                }
                            })
                        }
                    })
